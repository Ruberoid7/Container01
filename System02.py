...
status_leds = shiftpi2.SN74x595(ser_pin = PIN_SHIFT_STATUS_SER, rclk_pin = PIN_SHIFT_STATUS_RCLK, srclk_pin = PIN_SHIFT_STATUS_SRCLK)
diag_leds = shiftpi2.SN74x595(ser_pin = PIN_SHIFT_ERROR_SER, rclk_pin = PIN_SHIFT_ERROR_RCLK, srclk_pin = PIN_SHIFT_ERROR_SRCLK)
...
class LedControlPanel(object):
    ...
    def get_state_for_led(self, led):
        return led.state

    def get_state_for_led_num(self, led_num):
        return self[num].state

    def set_state_for_led(self, led, new_state):
        led.state = new_state
